...
const Home = () => {
        ...
        const fetchUserData = async () => {
            if (user) {
                ...
                if (docSnapshot.exists()) {
                    ...
                    const oldtime = data[14] || ''; // replace 'oldtime' with the actual key
                    setBestTime(oldtime);
                }
            }
        };
    ...
    const generateRandomNumbers = () => {
        const onesPlace = Math.floor(Math.random() * 20 + 90)
        setNum(onesPlace);
        const terms = Math.floor(Math.random() * 20 + 90);
        setRemainder(terms)
        setAns(terms * onesPlace)
    };
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (userAns === ("" + ans)) {
            ...
            if (problems == 4) {
                ...
                const docSnapshot = await getDoc(userDocumentRef);
                const data = docSnapshot.data();
                const oldtime = data ? (data[14] + "") : '';
                const newData = {
                    14: String(minutes).padStart(2, '0') + ":" + String(seconds).padStart(2, '0') + ":" + String(milliseconds + 1).substring(0, 2).padStart(2, '0')
                };
                if (timeCompare(oldtime, time)) {
                    updateDoc(userDocumentRef, newData)
                    const userDocumentRef2 = doc(db, 'leaderboard', '14' ?? '');
        ...
        return (
            <main className="h-screen overflow-auto w-screen  bg-orange-300">
                <div className="bg-white text-5xl p-10 text-orange-300 flex font-bold justify-center items-center">
                    ...
                    <div className="absolute text-2xl md:text-3xl">Number Sense: Multiplying 90-110</div>
