...
const Home = () => {
        ...
        const fetchUserData = async () => {
            if (user) {
                ...
                if (docSnapshot.exists()) {
                    ...
                    const oldtime = data[5] || ''; // replace 'oldtime' with the actual key
                    setBestTime(oldtime);
                }
            }
        };
    ...
    const generateRandomNumbers = () => {
        const nums = [9, 3, 8, 4, 5, 6];
        const num1 = Math.floor(Math.random() * 80000 + 11000);
        setNum(num1)
        const rem = nums[Math.floor(Math.random() * 6)]
        setRemainder(rem)
        setAns(num1 % rem)
    };
        ...
        return (
            <main className="h-screen overflow-auto w-screen  bg-orange-300">
                <div className="bg-white text-5xl p-10 text-orange-300 flex font-bold justify-center items-center">
                    ...
                    <div className="absolute text-2xl md:text-3xl">Number Sense: REMAINDER</div>
