...
const Home = () => {
        ...
        const fetchUserData = async () => {
            if (user) {
                ...
                if (docSnapshot.exists()) {
                    ...
                    const oldtime = data[4] || ''; // replace 'oldtime' with the actual key
                    setBestTime(oldtime);
                }
            }
        };
    ...
    const generateRandomNumbers = () => {
        const num1 = 111 * Math.floor(Math.random() * 750 + 120);
        setNum(num1)
        setAns(num1 / 111)
    };
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (userAns === ("" + ans)) {
            ...
            if (problems == 4) {
                ...
                const docSnapshot = await getDoc(userDocumentRef);
                const data = docSnapshot.data();
                const oldtime = data ? (data[4] + "") : '';
                const newData = {
                    4: time
                };
                if (timeCompare(oldtime, time)) {
                    updateDoc(userDocumentRef, newData)
                    const userDocumentRef2 = doc(db, 'leaderboard', '4' ?? '');
        ...
        return (
            <main className="h-screen overflow-auto w-screen  bg-orange-300">
                <div className="bg-white text-5xl p-10 text-orange-300 flex font-bold justify-center items-center">
                    ...
                    <div className="absolute text-2xl md:text-3xl">Number Sense: n/111</div>
