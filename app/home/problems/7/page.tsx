...
const Home = () => {
        ...
        const fetchUserData = async () => {
            if (user) {
                ...
                if (docSnapshot.exists()) {
                    ...
                    const oldtime = data[7] || ''; // replace 'oldtime' with the actual key
                    setBestTime(oldtime);
                }
            }
        };
    ...
    const generateRandomNumbers = () => {
        const num1 = Math.floor(Math.random() * 49000 + 1000);
        setNum(num1)
        const rem = Math.floor(Math.random() * 49000 + 1000);
        setRemainder(rem)
        setAns(num1 + rem)
    };
    useEffect(() => {
        const fetchData = async () => {
            if (userAns === ("" + ans)) {
                ...
                if (problems == 4) {
                    ...
                    const docSnapshot = await getDoc(userDocumentRef);
                    const data = docSnapshot.data();
                    const oldtime = data ? (data[7] + "") : '';
                    const newData = {
                        7: String(minutes).padStart(2, '0') + ":" + String(seconds).padStart(2, '0') + ":" + String(milliseconds + 1).substring(0, 2).padStart(2, '0')
                    };
                    if (timeCompare(oldtime, time)) {
                        updateDoc(userDocumentRef, newData)
                        const userDocumentRef2 = doc(db, 'leaderboard', '7' ?? '');
