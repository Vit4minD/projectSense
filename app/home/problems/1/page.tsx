...
const Home = () => {
    ...
    const [bestTime, setBestTime] = useState('test');
    useEffect(() => {
        ...
        const fetchUserData = async () => {
            if (user) {
                ...
                if (docSnapshot.exists()) {
                    ...
                    const oldtime = data[1] || ''; // replace 'oldtime' with the actual key
                    setBestTime(oldtime);
                }
            }
        };
    ...
    const generateRandomNumbers = () => {
        const num1 = Math.floor(Math.random() * 932 + 50);
        setNum(num1)
        setAns(num1 * 11)
    };
    ...
    let time = String(minutes).padStart(2, '0') + ":" + String(seconds).padStart(2, '0') + ":" + String(milliseconds).substring(0, 2).padStart(2, '0')
    if (problems < 5) {
        return (
            <main className="h-screen overflow-auto w-screen  bg-orange-300">
                <div className="bg-white text-5xl p-8 text-orange-300 flex font-bold justify-center items-center">
                    ...
                    <div className="absolute text-base md:text-4xl">Number Sense: n*11</div>
                    <div className=" text-xs md:text-4xl absolute right-8">{user ? user.email : 'Not logged in'}</div>
                </div>
                ...
                <form onSubmit={onSubmit} className="">
                    <div className="flex my-auto items-center text-9xl font-semibold text-white font-sans  w-full justify-center">
                        <div className=" text-xl md:text-8xl my-56">
                            {num} * 11 =
                            <input
                                autoFocus
                                className={`focus:cursor-auto ml-12 text-center text-xl md:text-8xl text-white bg-orange-300 border-b border-white focus:outline-none overflow-auto w-24 md:w-96`}
                                type="text"
